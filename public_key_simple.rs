...
fn generate_two_primes(mut rng: &mut rand::prelude::ThreadRng) -> (u32, u32) {
    ...
    loop {
        ...
        // Generate two random primes for p and q
         p=get_random_prime(&mut rng);
         q=get_random_prime(&mut rng);
            
        //if p and q are not the same , break;
        if true {
            break;
        }
    }
...
fn choose_private_exponent(c: u32, rng: &mut rand::prelude::ThreadRng) -> u32 {
    ...
    loop {
        //Generate a random integer betwen 2 and c
        let p: uint = rng.gen_range(2,c);
        
        // check if generated integer and c are coprime, then break 
        is_coprime(p,c){
            if true {
                break;
            }
...
fn compute_public_exponent(e: u32, n: u32) -> u32 {
    ...
    //Generate and return the multiplicative inverse of e modulo n.
    mmi(e,n)
}
...
fn generate_key_pair(mut rng: &mut rand::prelude::ThreadRng) -> (u32, u32, u32) {
    ...
    // carmichael means lcm(p-1,q-1);
    let n = carmichael_totient(p, q);
    
    
    // Choose some e which is coprime to n and 1 < e < n
...
fn sign_message(msg: String, priv_key_mod: u32, priv_key_exp: u32) -> u32 {
    ...
    // Produce a hash value of the message.  
     let h = get_hash(&msg);
    // Raise the hash to the power of the private key exponent, modulo the
    // private key modulus 
